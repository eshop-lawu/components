...
=====
```bash
nohup java -jar id-worker-1.0-SNAPSHOT.jar --lawu.id-worker-generate.worker-id=0 --eureka.client.serviceUrl.defaultZone=http://192.168.1.26:8888/eureka/ &
```
...
docker run -d --name id-worker -p 9002:9002 \
    ...
    -e lawu.id-worker-generate.worker-id=0 \
...
docker run -d --name id-worker -p 9002:9002 \
    ...
    -e TZ=Asia/Shanghai
    -e eureka.client.serviceUrl.defaultZone=http://192.168.100.183:8888/eureka/,http://192.168.100.184:8888/eureka/ \
    -e lawu.id-worker-generate.worker-id=1 \
    registry.eshop.com/library/id-worker
```
